//
//  AuthorizationRequestTestsBase.cs
//
//  Author:
...
/// <summary>
/// Serves as a test base for test classes that handle authorization requests.
/// </summary>
/// <typeparam name="TAuthorizationRequest">The request type.</typeparam>
public abstract class AuthorizationRequestTestsBase<TAuthorizationRequest> where TAuthorizationRequest : IAuthorizationRequest
{
    /// <summary>
    /// Gets the expected response type.
    /// </summary>
    protected abstract string ResponseType { get; }
    ...
    /// </summary>
    protected abstract Func<IAuthorizationRequestExtension, TAuthorizationRequest> ExtendedRequestFactory { get; }

    /// <summary>
    /// Gets the expected HTTP method that the request should use.
    /// </summary>
    protected virtual HttpMethod ExpectedHttpMethod => HttpMethod.Get;

    /// <summary>
    /// Tests whether the created request includes the correct response type.
    /// </summary>
    [Fact]
    public void CreatesRequestWithCorrectResponseType()
    {
        var request = this.BlankRequestFactory();
        var message = request.ToRequest(new Uri("about:blank"));

        Assert.Equal(this.ResponseType, request.ResponseType);
